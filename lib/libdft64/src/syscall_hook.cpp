...
/**
 * The original implementation tag taints to the first argument of mmap. 
 * However, the kernel just takes the first argument as a hint about where to place the mapping. 
 * In other words, the first argument may be not the final address to place the mapping.
 * 
 * Therefore, the proper address to tag taint should be the return value of the mmap function. 
 */
...
static void post_mmap_hook(THREADID tid, syscall_ctx_t *ctx) {
  ...
  // fprintf(stderr, "[mmap] fd: %d(%d), addr: %lx, readoff: %ld, nr:%ld \n", fd,
  //       is_fuzzing_fd(fd), buf, read_off, nr);
  if (is_fuzzing_fd(fd)) {
    tainted = true;
    LOGD("[mmap] fd: %d, addr: %lx, offset: %ld, size: %lu\n", fd, ret, read_off, nr);
